...
const COLLISION_PAIR_TEST_SIZES = TEST_SIZES.reduce<TestVariationBodySizePair[]>((acc, testSize) => {
    acc.push(
        ...TEST_SIZES.reduce<TestVariationBodySizePair[]>((acc, otherTestSize) => {
            acc.push({
...
const TEST_VARIATIONS = Object.values(Direction).reduce<TestVariation[]>((acc, dir) => {
    acc.push(
        ...Object.values(CollisionType).reduce<TestVariation[]>((acc, collisionType) => {
            acc.push(
                ...COLLISION_PAIR_TEST_SIZES.reduce<TestVariation[]>((acc, collisionPairTestSize) => {
                    acc.push({
...
describe('CollisionEvents', () => {
    describe('getCollisionEventsInChronologicalOrder()', () => {
        let getCollisionEventsInChronologicalOrder: () => ReturnType<
            typeof CollisionEvents['getCollisionEventsInChronologicalOrder']
        >;
        ...
        for (const testVariation of TEST_VARIATIONS) {
            ...
            describe(`${collisionType} (${movingBodySize} -> ${direction} -> ${collisionBodySize})`, () => {
                ...
                beforeEach(() => {
                    ...
                    getCollisionEventsInChronologicalOrder = () =>
                        CollisionEvents.getCollisionEventsInChronologicalOrder(bodies, movingBody);
                ...
                describe("tangential movement (don't recognize collision events for grazing bodies)", () => {
                    ...
                    it(`${SHOULD_NOT} when a body moves tangentially to another body`, () => {
                        const tangentialVectors = TestUtils.getTangentialMovementVectors(
                            collisionPair,
                            direction,
                            getDiffPos,
                        );

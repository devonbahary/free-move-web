...
import { CircleVsCircleCollisionEvent, CircleVsRectangleCollisionEvent, RectangleVsRectangleCollisionEvent } from "./CollisionEvent";
...
export class Collisions {
    ...
    static getCircleVsRectangleCollisionEvents = (circle, rect) => {
        ...
        const timeOfX0Collision = Collisions.getTimeOfAxisAlignedCollision(x + radius, x0, dx);
        ...
        const timeOfX1Collision = Collisions.getTimeOfAxisAlignedCollision(x - radius, x1, dx);
        ...
        const timeOfY0Collision = Collisions.getTimeOfAxisAlignedCollision(y + radius, y0, dy);
        ...
        const timeOfY1Collision = Collisions.getTimeOfAxisAlignedCollision(y - radius, y1, dy);
        ...
        for (const corner of corners) {
            const cornerTimeOfCollision = Collisions.getTimeOfCircleCollisionWithRectangleCorner(circle, corner);
    ...
    static getRectangleVsCircleCollisionEvents = (rect, circle) => {
        ...
        const getCirclePointYsFromCircleX = (x) => {
            ...
            return quadratic(1, b, c).filter(a => !isNaN(a));
        };
    ...
    static resolveCollision = (collisionEvent) => {
        ...
        if (collisionBody.isFixed) {
            if (Collisions.isCircleVsCircle(movingBody, collisionBody)) {
                ...
                if (roundFloatingPoint(diffPositions.x) === 0) {
                    movingBody.setVelocity({ x: vA.x, y: -vA.y });
                } else if (roundFloatingPoint(diffPositions.y === 0)) {
                    movingBody.setVelocity({ x: -vA.x, y: vA.y });
                } else {
                    movingBody.setVelocity(Vectors.rescale(diffPositions, Vectors.magnitude(vA)));
                }
